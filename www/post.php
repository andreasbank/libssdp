...
foreach($abused_results as $abused_result) {
  try {
    ...
    echo "Is probed: ".($is_probed ? 'true' : 'false')."<br />\n";

    /* If device has not been probed for capabilities
       then probe it and fill the database */
    if(!$is_probed) {
      if($cm->has_sd_disk()) {
        $capability_id = $sqlconnection->call(sprintf("call add_capability_if_not_exist('%s', '%s');",
                                                      'ptz',
                                                      'Properties.LocalStorage.SDCard'))[0][0]['id'];
        echo "Cap_id: ".$capability_id."<br />\n";
