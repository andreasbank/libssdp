...
if(isset($_GET['test_mysql'])) {
  ...
  try {
    ...
    $query = sprintf("%s       ON mfcs.`capability_id` = c.`id`\n", $query);
    $query = sprintf("%s       AND mfcs.`state` LIKE v_capability_state\n", $query);
    $query = sprintf("%s  WHERE d.`model_firmware_id`=mf.`id`\n", $query);
    $query = sprintf("%s  AND IF(v_capability_state = '%',\n", $query);
    $query = sprintf("%s         (mfcs.`model_firmware_id` IS NULL\n", $query);
    $query = sprintf("%s         OR mfcs.`model_firmware_id` = d.`model_firmware_id`),\n", $query);
    $query = sprintf("%s         mfcs.`model_firmware_id` = d.`model_firmware_id`)\n", $query);
    ...
    query($sql, $query);
    $query = "GRANT SELECT, INSERT, UPDATE ON `abused`.`model_firmware_capability_state` TO 'abused'@'%';";
    query($sql, $query);
    ...
    query($sql, $query);
    $query = "GRANT EXECUTE ON PROCEDURE abused.add_or_update_model_firmware_capability_state TO 'abused'@'%';";
    query($sql, $query);
