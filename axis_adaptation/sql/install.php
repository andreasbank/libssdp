...
if(isset($_GET['test_mysql'])) {
  ...
  try {
    ...
    $query = sprintf("%s         (dcs.`device_id` IS NULL\n", $query);
    $query = sprintf("%s         OR dcs.`device_id` = d.`id`),\n", $query);
    $query = sprintf("%s         dcs.`device_id` = d.`id`)\n", $query);
    ...
    $query = sprintf("%s    CALL `list_devices`(NULL, NULL, NULL, NULL, 999999, NULL, v_found_id, NULL);\n", $query);
    $query = sprintf("%s  END IF;\n", $query);
    $query = sprintf("%sEND;\n", $query);
    query($sql, $query);
    ...
    $query = sprintf("%s            LEFT OUTER JOIN `device_capability_state` dcs\n", $query);
    $query = sprintf("%s            ON mfc2.`capability_id` = dcs.`capability_id`,\n", $query);
    $query = sprintf("%s                 `capabilities` c2\n", $query);
    $query = sprintf("%s            WHERE mfc2.`capability_id` = c2.`id`\n", $query);
    $query = sprintf("%s            AND mfc2.`model_firmware_id` = d.`model_firmware_id`\n", $query);
