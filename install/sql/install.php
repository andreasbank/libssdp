...
function resultset_to_array($resultset) {
  $results = array();
...
if(isset($_GET['test_mysql'])) {
  $sql = new mysqli($_GET['h'], $_GET['u'], $_GET['p'], $_POST['databaseName']);
  ...
  try {
    ...
    $query = sprintf("`model_name` VARCHAR(255) NOT NULL,\n", $query);
    $query = sprintf("`firmware_version` VARCHAR(255) NOT NULL,\n", $query);
    $query = sprintf("`last_updated` DATETIME,\n", $query);
    $query = sprintf("UNIQUE KEY(`model_name`, `firmware_version`),\n", $query);
    $query = sprintf("PRIMARY KEY(`id`)) ENGINE=InnoDB;\n", $query);
    query($sql, $query);

    /* model_firmware_capability table */
    $query =         "CREATE TABLE `model_firmware_capability` (`model_firmware_id` INT NOT NULL,\n";
    $query = sprintf("`capability_id` INT NOT NULL,\n", $query);
    $query = sprintf("FOREIGN KEY (`model_firmware_id`)\n", $query);
    $query = sprintf("REFERENCES `model_firmware`(`id`),\n", $query);
    $query = sprintf("FOREIGN KEY (`capability_id`)\n", $query);
    $query = sprintf("REFERENCES `capabilities`(`id`),\n", $query);
    $query = sprintf("PRIMARY KEY(`model_firmware_id`, `capability_id`)) ENGINE=InnoDB;\n", $query);
    query($sql, $query);
 
    /* devices table */
    $query =         "CREATE TABLE `devices` (`id` VARCHAR(255) NOT NULL,\n";
    $query = sprintf("`mac` VARCHAR(255) NOT NULL,\n", $query);
    $query = sprintf("`ipv4` VARCHAR(15),\n", $query);
    $query = sprintf("`ipv6` VARCHAR(46),\n", $query);
    $query = sprintf("`friendly_name` VARCHAR(255),\n", $query);
    $query = sprintf("`model_firmware_id` INT,\n", $query);
    $query = sprintf("`last_update` DATETIME NOT NULL,\n", $query);
    $query = sprintf("`last_upnp_message` ENUM('hello', 'alive', 'bye') NOT NULL,\n", $query);
    $query = sprintf("FOREIGN KEY (`model_firmware_id`) REFERENCES `model_firmware` (`id`),\n", $query);
    $query = sprintf("PRIMARY KEY (`id`)) ENGINE=InnoDB;\n", $query);
    query($sql, $query);

    /* locked_devices table */
    $query =         "CREATE TABLE `locked_devices`(`device_id` VARCHAR(255) NOT NULL,\n";
    $query = sprintf("`locked` TINYINT(1) DEFAULT 1,\n", $query);
    $query = sprintf("`locked_by` VARCHAR(255) NOT NULL,\n", $query);
    $query = sprintf("`locked_date` DATETIME NOT NULL,\n", $query);
    $query = sprintf("FOREIGN KEY (`device_id`)\n", $query);
    $query = sprintf("REFERENCES `devices`(`id`)\n", $query);
    $query = sprintf("ON DELETE CASCADE\n", $query);
    $query = sprintf("ON UPDATE CASCADE,\n", $query);
    $query = sprintf("PRIMARY KEY (`device_id`, `locked_date`)) ENGINE=InnoDB;\n", $query);
    query($sql, $query);
    ...
    $query =         "%sCREATE PROCEDURE `add_or_update_device`(IN `v_id` VARCHAR(255),\n";
    ...
    $query =         "%sCREATE PROCEDURE `add_model_firmware_if_not_exist`(IN `v_model_name` VARCHAR(255),\n";
    ...
    $query =         "%sCREATE PROCEDURE `is_model_firmware_probed`(IN `v_model_firmware_id` INT)\n";
    $query = sprintf("%s SQL SECURITY INVOKER\n", $query);
    $query = sprintf("%sBEGIN\n", $query);
    ...
    $query =         "%sCREATE PROCEDURE `add_capability_to_model_firmware` (IN `v_model_firmware_id` INT,\n";
    ...
    $query =         "%sCREATE PROCEDURE `delete_inactive_devices`(IN `inactive_seconds` INT)\n";
    $query = sprintf("%s SQL SECURITY INVOKER\n", $query);
    $query = sprintf("%sBEGIN\n", $query);
    ...
    $query =         "%sCREATE PROCEDURE `is_device_locked_internal`(IN `v_device_id` VARCHAR(255),\n";
    ...
    /* lock_device */
    $query =         "%sCREATE PROCEDURE `lock_device`(IN `v_device_id` VARCHAR(255),\n";
    ...
    /* inlock_device */
    $query =         "%sCREATE PROCEDURE `unlock_device`(IN `v_device_id` VARCHAR(255))\n";
    $query = sprintf("%s SQL SECURITY INVOKER\n", $query);
    $query = sprintf("%sBEGIN\n", $query);
    ...
    /* is_device_locked */
    $query =         "%sCREATE PROCEDURE `is_device_locked` (IN `v_device_id` INT)";
